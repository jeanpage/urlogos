    ...
    class TGM_Plugin_Activation{
        ...
        public function __construct(){
            ...
            $this->strings = array(
                ...
                'oops' => esc_html__('Something went wrong.','liquid'),
                ...
                'activate_link' => _n_noop('Begin activating plugin','Begin activating plugins','liquid'),
                'return' => esc_html__('Return to Required Plugins Installer','liquid'),
                'dashboard' => esc_html__('Return to the dashboard','liquid'),
                'plugin_activated' => esc_html__('Plugin activated successfully.','liquid'),
                'activated_successfully' => esc_html__('The following plugin was activated successfully:','liquid'),
                'complete' => esc_html__('All plugins installed and activated successfully. %1$s','liquid'),
                'dismiss' => esc_html__('Dismiss this notice','liquid'),
           );
        ...
        protected function do_plugin_install(){
            ...
            if (isset($_GET['plugin'])&&(isset($_GET['tgmpa-install'])&& 'install-plugin' == $_GET['tgmpa-install'])){
                ...
                if (empty($complete)){
                    echo '<p>'.sprintf($this->strings['complete'],'<a href="'.network_admin_url() .'" title="' . esc_html('Return to the Dashboard').'">'.esc_html('Return to the Dashboard').'</a>').'</p>';
        ...
        public function notices(){
            ...
            if (! empty($message)){
                ...
                foreach($message as $type => $plugin_groups){
                    ...
                    $rendered .= '<p>' . sprintf(translate_nooped_plural($this->strings[$type],$count,'liquid'),$imploded,$count) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg('page',$this->menu,network_admin_url('themes.php')) . '">' . translate_nooped_plural($this->strings['install_link'],$install_link_count,'liquid') . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg('page',$this->menu,network_admin_url('themes.php')) . '">' . translate_nooped_plural($this->strings['activate_link'],$activate_link_count,'liquid') . '</a>'  : '';
                ...
                if (! empty($this->strings['nag_type'])){
                    add_settings_error('liquid','liquid',$rendered,sanitize_html_class(strtolower($this->strings['nag_type'])));
                } else{
                    add_settings_error('liquid','liquid',$rendered,$nag_class);
                }
            }

            // Admin options pages already output settings_errors,so this is to avoid duplication.
            if ('options-general' !== $current_screen->parent_base){
                settings_errors('liquid');
            }
...
if (! class_exists('TGMPA_List_Table')){
    ...
    class TGMPA_List_Table extends WP_List_Table{
            ...
            foreach(TGM_Plugin_Activation::$instance->plugins as $plugin){
                ...
                if (! empty($plugin['source'])){
                    ...
                    if (preg_match('|^http(s)?://|',$plugin['source'])){
                        $table_data[$i]['source'] = esc_html('Private Repository');
                    // The plugin is pre-packaged with the theme.
                    } else{
                        $table_data[$i]['source'] = esc_html('Pre-Packaged');
                    }
                }
                // The plugin is from the WordPress repository.
                else{
                    $table_data[$i]['source'] = esc_html('WordPress Repository');
                }

                $table_data[$i]['type'] = isset($plugin['required']) && $plugin['required'] ? esc_html('Required') : esc_html('Recommended');

                if (! isset($installed_plugins[$plugin['file_path']])){
                    $table_data[$i]['status'] = sprintf('%1$s',esc_html('Not Installed'));
                } elseif (is_plugin_inactive($plugin['file_path'])){
                    $table_data[$i]['status'] = sprintf('%1$s',esc_html('Installed But Not Activated'));
                }
        ...
        public function no_items(){
            ...
            printf(esc_html('No plugins to install or activate.'),network_admin_url());
            echo '<style type="text/css">#adminmenu .wp-submenu li.current{ display: none !important; }</style>';
        ...
        public function process_bulk_actions(){
            ...
            if ('tgmpa-bulk-activate' === $this->current_action()){
                ...
                if (is_wp_error($activate)){
                    ...
                    printf('<div id="message" class="updated"><p>%1$s %2$s</p></div>',_n('The following plugin was activated successfully:','The following plugins were activated successfully:',$count,'liquid'),$imploded);
                }
    ...
    if (! class_exists('TGM_Bulk_Installer')){
        ...
        class TGM_Bulk_Installer extends WP_Upgrader{
            ...
            public function install_strings(){
                ...
                $this->strings['no_package']          = esc_html('Install package not available.');
                $this->strings['downloading_package'] = esc_html('Downloading install package from <span class="code">%s</span>&#8230;');
                $this->strings['unpack_package']      = esc_html('Unpacking the package&#8230;');
                $this->strings['installing_package']  = esc_html('Installing the plugin&#8230;');
                $this->strings['process_failed']      = esc_html('Plugin install failed.');
                $this->strings['process_success']     = esc_html('Plugin installed successfully.');
            ...
            public function activate_strings(){
                ...
                $this->strings['activation_failed']  = esc_html('Plugin activation failed.','liquid');
                $this->strings['activation_success'] = esc_html('Plugin activated successfully.','liquid');
    ...
    if (! class_exists('TGM_Bulk_Installer_Skin')){
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin{
            ...
            public function add_strings(){
                ...
                if (TGM_Plugin_Activation::$instance->is_automatic){
                    $this->upgrader->strings['skin_upgrade_start'] = esc_html('The installation and activation process is starting. This process may take a while on some hosts,so please be patient.');
                    $this->upgrader->strings['skin_update_successful']    = esc_html('%1$s installed and activated successfully.') . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html('Show Details') . '</span><span class="hidden">' . esc_html('Hide Details') . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = esc_html('All installations and activations have been completed.');
                    $this->upgrader->strings['skin_before_update_header'] = esc_html('Installing and Activating Plugin %1$s (%2$d/%3$d)');
                }
                // Default installation strings.
                else{
                    $this->upgrader->strings['skin_upgrade_start']        = esc_html('The installation process is starting. This process may take a while on some hosts,so please be patient.');
                    $this->upgrader->strings['skin_update_failed_error']  = esc_html('An error occurred while installing %1$s: <strong>%2$s</strong>.');
                    $this->upgrader->strings['skin_update_failed']        = esc_html('The installation of %1$s failed.');
                    $this->upgrader->strings['skin_update_successful']    = esc_html('%1$s installed successfully.') . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . esc_html('Show Details') . '</span><span class="hidden">' . esc_html('Hide Details') . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = esc_html('All installations have been completed.');
                    $this->upgrader->strings['skin_before_update_header'] = esc_html('Installing Plugin %1$s (%2$d/%3$d)');
                }
            }
            ...
            public function bulk_footer(){
                ...
                if (empty($complete)){
                    echo '<p>' .  sprintf(TGM_Plugin_Activation::$instance->strings['complete'],'<a href="' . network_admin_url().'" title="'.esc_html('Return to the Dashboard').'">'.esc_html('Return to the Dashboard').'</a>').'</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current{ display: none !important; }</style>';
                }
